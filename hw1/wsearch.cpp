...
bool readGrid(istream& instream, vector<vector<char> >& grid )
{
   ...
   string myline;
   //for each line, turn to stream
   //and then get every cahracter from each line
   while( getline(instream, myline) ){
      stringstream ss(myline);
      string chara;
      //add each char to a vector
      vector<char> temp;
      while( ss>>chara )
      { 
         temp.push_back(chara[0]);
      }
      grid.push_back(temp);
   }
   //error checking: if size is off
   //or not uniform
   if(grid[0].size() <= 0) return false;
...
bool findWordHelper(const vector<vector<char> >& grid, 
		 ...
		 unsigned int currWordIndex)
{
      //run until the word size found is the same as the search
      // word target
      unsigned int wsize = word.size();
      if(wsize == currWordIndex){
            return true;
            }
      //return false if we go off the grid
      if(currLoc.row <0 || currLoc.col < 0 )
            return false;
      if(currLoc.row >= grid.size() || currLoc.col >= grid[0].size() )
            return false;
       //if the letters match, run the function again
